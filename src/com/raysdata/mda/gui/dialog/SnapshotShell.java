...
public class SnapshotShell extends Canvas implements MouseMoveListener, MouseListener, PaintListener, DragSourceListener, DropTargetListener {
	...
	public SnapshotShell(Composite composite, Image bi) {
		...
		//try {
			//FileInputStream imageIo = new FileInputStream("image/clean.gif");
			//bagground = new Image(getDisplay(), imageIo);
			bagground = UIBuilderFactory.createImage(ResourceConstant.IMAGE_SNAPSHOT_CLEAN);

			//imageIo = new FileInputStream("image/save.gif");
			//saveImages = saveImage = new Image(getDisplay(), imageIo);
			saveImages = saveImage = UIBuilderFactory.createImage(ResourceConstant.IMAGE_SNAPSHOT_SAVE);
			
			//imageIo = new FileInputStream("image/copy.gif");
			//copyImages = copyImage = new Image(getDisplay(), imageIo);
			copyImages = copyImage = UIBuilderFactory.createImage(ResourceConstant.IMAGE_SNAPSHOT_COPY);

			//imageIo = new FileInputStream("image/clean.gif");
			//cleanImages = cleanImage = new Image(getDisplay(), imageIo);
			cleanImages = cleanImage = UIBuilderFactory.createImage(ResourceConstant.IMAGE_SNAPSHOT_CLEAN);

			//imageIo = new FileInputStream("image/saves.png");
			//newSaveImages = new Image(getDisplay(), imageIo);
			newSaveImages = UIBuilderFactory.createImage(ResourceConstant.IMAGE_SNAPSHOT_SAVES);
			
			//imageIo = new FileInputStream("image/copy.gif");
			//newCopyImages = new Image(getDisplay(), imageIo);
			newCopyImages = UIBuilderFactory.createImage(ResourceConstant.IMAGE_SNAPSHOT_COPY);

			//imageIo = new FileInputStream("image/clean.gif");
			//newCleanImages = new Image(getDisplay(), imageIo);
			newCleanImages = UIBuilderFactory.createImage(ResourceConstant.IMAGE_SNAPSHOT_CLEAN);
//		} catch (IOException e) {
//			e.printStackTrace();
//		}
		 ...
		 * ds.addDragListener(this); DropTarget dt=new
		 * DropTarget(this,DND.DROP_MOVE); dt.addDropListener(this);
		 */
		initRecs();
	}
	...
	public void mouseMove(MouseEvent me) {
		...
		// }
		/*
		 * if(showTip){ p=new Point(me.x,me.y); this.redraw(); }
		 */
	}
	...
	public void doCopy(final Image get) {
		try {
			...
			openMessageBox(this.getShell(), "提示", "已复制到系统粘帖板!!");
		} catch (Exception exe) {
			exe.printStackTrace();
	...
	public void doSave(Image get) {
		try {
			...
			if (savePath != null) {
				...
				loader.save(savePath, SWT.IMAGE_PNG);
			}
		} catch (Exception exe) {
			exe.printStackTrace();
		}
	...
	public void paintControl(PaintEvent e) {
		...
		g.setBackground(getDisplay().getSystemColor(SWT.COLOR_DARK_BLUE));
		g.setForeground(getDisplay().getSystemColor(SWT.COLOR_DARK_BLUE));
		...
		copyRect = new Rectangle(saveRect.x + copyImage.getBounds().width + 2, rect.y + 2, copyImage.getBounds().width, copyImage.getBounds().height);
		...
		cleanRect = new Rectangle(copyRect.x + cleanImage.getBounds().width + 2, rect.y + 2, cleanImage.getBounds().width, cleanImage.getBounds().height);
	...
	enum States {
		...
		NORTH(Display.getCurrent().getSystemCursor(SWT.CURSOR_SIZEN)), NORTH_EAST(Display.getCurrent().getSystemCursor(SWT.CURSOR_SIZENE)), EAST(Display.getCurrent().getSystemCursor(SWT.CURSOR_SIZEE)), SOUTH_EAST(
				Display.getCurrent().getSystemCursor(SWT.CURSOR_SIZESE)), SOUTH(Display.getCurrent().getSystemCursor(SWT.CURSOR_SIZES)), SOUTH_WEST(Display.getCurrent().getSystemCursor(
				SWT.CURSOR_SIZESW)), WEST(Display.getCurrent().getSystemCursor(SWT.CURSOR_SIZEW)), MOVE(Display.getCurrent().getSystemCursor(SWT.CURSOR_SIZEALL)), DEFAULT(Display.getCurrent()
